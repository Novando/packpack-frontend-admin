                ...
                <table class="table table-hover text-nowrap">
                  ...
                  <tbody>
                    <tr v-for="attribute in attributes" :key="attribute.id">
                      <td>{{ attribute.invoice }}</td>
                      <td>{{ attribute.email }}</td>
                      <td>{{ attribute.phone }}</td>
                      <td>{{ attribute.totalPrice }}</td>
                      <td>{{ attribute.createdAt }}</td>
											<td>{{ attribute.status }}</td>
                      <td>{{ attribute.modifiedBy }}</td>                      <td>{{ attribute.updatedAt }}</td>
											<td>
...
<script>
	...
	export default {
		...
		async mounted() {
			this.attributes = (await OrderController.print()).data
		},

		methods: {
      async openEdit(id){
        this.edit = (await OrderController.show(id)).data
      },

      async editMaterial(id){
        await OrderController.edit(id, {
					id 					: this.edit.id,
					name				: this.edit.name,
      ...
      async restock(id){
        await OrderController.restock(id, {
					stock 		: this.stock,
        })
				this.attributes = (await OrderController.print()).data
      },
      ...
      async adjust(id){
        await OrderController.adjust(id, {
